...
class nn_mnist_classifier:
    def __init__(self, mmt_friction=0.9, lr=1e-2):
        ...
        self.conv_layer_1 = nnl.nn_convolutional_layer(Wx_size=3, Wy_size=3, input_size=28, in_ch_size=1, out_ch_size=28)
    ...
    def update_weights(self):
        ...
        self.v_w_fc2 = dldw_fc2*(1-friction) + friction*self.v_w_fc2
        self.v_b_fc2 = dldb_fc2*(1-friction) + friction*self.v_b_fc2

        self.v_w_fc1 = dldw_fc1*(1-friction) + friction*self.v_w_fc1
        self.v_b_fc1 = dldb_fc1*(1-friction) + friction*self.v_b_fc1

        self.v_w_cv1 = dldw_cv1*(1-friction) + friction*self.v_w_cv1
        self.v_b_cv1 = dldb_cv1*(1-friction) + friction*self.v_b_cv1

        # using v, perform weight update for each layer
        self.fc2.update_weights(-1*self.v_w_fc2*lr, -1*self.v_b_fc2*lr)
        self.fc1.update_weights(-1*self.v_w_fc1*lr, -1*self.v_b_fc1*lr)
        self.conv_layer_1.update_weights(-1*self.v_w_cv1*lr, -1*self.v_b_cv1*lr)
        ########################
        # Q3 ends here
        ########################
